...
def partition(data, first, last):
    ...
    while not done:
        while leftmark <= rightmark and data[leftmark] >= pivotvalue: # Dalam kasus ini, kita cukup menukar pembanding dari pivotvalue ini, yang awalnya <= menjadi >=
            leftmark = leftmark + 1

        while data[rightmark] <= pivotvalue and rightmark >= leftmark: # Dan sebaliknya dengan pembanding dari pivotvalue ini, yang awalnya >= menjadi <=
